...
| `Conv`                      | PyTorch `nn.Conv1d`, Keras `Conv1D`                          | 1-D (weight rank 3) only; `group=1` only; `auto_pad` NOTSET/VALID/SAME_UPPER/SAME_LOWER                                                                                                                                     |
...
| `MeanVarianceNormalization` | MVN-normalisation in preprocessing pipelines                 | Normalises over feature axis; `axes` attribute supported                                                                                                                                                                    |
...
- **Vanilla RNN** (`RNN` operator — supported for forward direction single-layer only; backward/bidirectional and `sequence_lens` raise `NotImplementedError`)
