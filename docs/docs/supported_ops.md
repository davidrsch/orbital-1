...
| `LSTM`                      | PyTorch `nn.LSTM`, Keras `LSTM`                              | Forward direction only; IOFC gate order; zero initial state; no `sequence_lens`; no peephole; `input_forget=0` only                                                                                                         |
...
- **Attention / self-attention** (`Attention`) — standard ONNX `Attention` op is not translatable from ONNX. Note: the R orbital package supports `MultiHeadAttention`, `AdditiveAttention`, `GroupedQueryAttention`, and `Attention` directly from Keras3 models via the DAG path. The Python package supports the `com.microsoft.MultiHeadAttention` contrib op.
