...
- **Attention / self-attention** (`Attention`, `MultiHeadAttention`) — not translatable from ONNX. Note: the R orbital package supports `MultiHeadAttention` directly from Keras3 models via the DAG path.
- **Vanilla RNN** (`RNN` operator — raises `KeyError`; only `LSTM` and `GRU` are currently supported; `nn.RNN` / `SimpleRNN` models that export an ONNX `RNN` node are not translatable)
