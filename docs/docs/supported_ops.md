...
| `AveragePool`               | PyTorch `nn.AvgPool1d`, `nn.AdaptiveAvgPool1d`               | 1-D only; kernel_shape=1 pass-through; sliding window (`k < L`) and global (`k == L`) supported; `auto_pad` NOTSET/VALID/SAME_UPPER/SAME_LOWER; `ceil_mode`; `count_include_pad`; dilations > 1 raise `NotImplementedError` |
...
| `Conv`                      | PyTorch `nn.Conv1d`, Keras `Conv1D`                          | 1-D (weight rank 3) only; `group=1` (standard) or `group==C_in` (depthwise) supported; any other `group` value raises; `auto_pad` NOTSET/VALID/SAME_UPPER/SAME_LOWER                                                        |
| `ConvTranspose`             | PyTorch `nn.ConvTranspose1d`, decoders                       | 1-D (weight rank 3) only; `group=1`; `output_padding`; `auto_pad` NOTSET/VALID/SAME_UPPER/SAME_LOWER; dilations > 1 raise `NotImplementedError`                                                                             |
...
| `MaxPool`                   | PyTorch `nn.MaxPool1d`                                       | 1-D only; kernel_shape=1 pass-through; sliding window (`k < L`) and global (`k == L`) supported; `auto_pad` NOTSET/VALID/SAME_UPPER/SAME_LOWER; `ceil_mode`; dilations > 1 raise `NotImplementedError`                      |
...
- **Vanilla RNN** (`RNN` operator — `direction="forward"` and `direction="bidirectional"` supported; `direction="reverse"` alone and `sequence_lens` raise `NotImplementedError`)
- **Stateful RNN** (`LSTM`/`GRU` with `sequence_lens`, non-zero `initial_h`/`initial_c`, or `stateful=True`)
- **Dynamic control flow** (`Loop`, `If`, `Scan`)
- **Spatial pooling** (`GlobalAveragePool`/`GlobalMaxPool` 2D/3D, `AveragePool`/`MaxPool` with dilations > 1 or 2D/3D kernels)
